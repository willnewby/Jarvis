...
public class SpeechInterface {
	private static final Logger logger = Logger
			.getLogger(SpeechInterface.class);
	...
	public static void init(String vocePath, boolean initSynthesis,
		...
		if (!initSynthesis && !initRecognition) {
			logger.warn("Synthesizer and recognizer are both"
					+ "uninitialized.");
		}
		...
		if (initRecognition) {
			...
			logger.info("Initializing recognizer. This may take some time...");
			mRecognizer = new SpeechRecognizer(vocePath + "/" + configFilename,
	...
	public static void synthesize(String message) {
		if (null == mSynthesizer) {
			logger.warn("synthesize called before "
	...
	public static void stopSynthesizing() {
		if (null == mSynthesizer) {
			logger.warn("stopSynthesizing called before "
					+ "synthesizer was initialized.  Request will be ignored.");
			return;
		}
	...
	public static int getRecognizerQueueSize() {
		if (null == mRecognizer) {
			logger.warn("getRecognizerQueueSize "
					+ "called before recognizer was initialized.  Returning 0.");
			return 0;
		}
	...
	public static String popRecognizedString() {
		if (null == mRecognizer) {
			logger.warn("popRecognizedString "
	...
	public static void setRecognizerEnabled(boolean e) {
		if (null == mRecognizer) {
			logger.warn("setRecognizerEnabled called before recognizer was initialized.  Request will be ignored.");
			return;
		}
	...
	public static boolean isRecognizerEnabled() {
		if (null == mRecognizer) {
			logger.warn("isRecognizerEnabled called before recognizer was initialized.  Returning false.");
			return false;
		}
