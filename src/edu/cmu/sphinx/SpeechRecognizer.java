...
public class SpeechRecognizer implements Runnable {
	...
	private static final Logger logger = Logger
			.getLogger(SpeechRecognizer.class);
	...
	public SpeechRecognizer(String configFilename, String grammarPath,
		...
		try {
			...
			if (!grammarName.equals("")) {
				...
				logger.warn("No grammar file specified.  Defaulting to 'digits.gram'");
			}
	...
	public void run() {
		...
		while (true == mRecognitionThreadEnabled) {
			if (!mMicrophone.isRecording()) {
				logger.warn("Recognition thread is running, but the microphone is disabled.");
			} else {
				...
				if (result != null) {
					...
					if (!s.equals("")) {
						logger.debug("Finished recognizing");
	...
	public void setEnabled(boolean e) {
		if (e) {
			...
			if (!success) {
				logger.warn("Cannot initialize microphone. Speech recognition disabled.");
				return;
			} else {
				if (null != mRecognitionThread) {
					logger.warn("New recognition thread being created before the previous one finished.");
				}
